...
function camxes_postprocessing(text, mode) {
	...
	text = text.replace(/(gismu|lujvo|fuhivla|cmene|cmevla),([A-Za-zàèìòùŭĭ']+)/g, "$1:$2");
	text = text.replace(/([A-Za-zàèìòùŭĭ_]+),([A-Za-zàèìòùŭĭ']+)/g, "$1:$2");
	...
	text = text.replace(/([A-Za-zàèìòùŭĭ0-9_]+,)+\[/g, "[");
	text = text.replace(/,/gm, "");
	...
	if (!with_selmaho) text = text.replace(/[A-Za-zàèìòùŭĭ_]+:/g, "");
	text = text.replace(/\]\[/g, "] [");
...
function erase_elided_terminators(str) {
	var i, j;
	var parachute = 400;
	str = str.replace(/([a-zàèìòùŭĭ']+)`/g, function(v) { return v.toUpperCase(); });
...
function delete_superfluous_brackets(string) {
	...
	**/
	do {
		...
		var j = i;
		while (str[i].search(/[A-Za-zàèìòùŭĭ'_:~]/) === 0 && i < str.length) i++;
		while (str[i] == ']') {
			if (j <= 0) {
